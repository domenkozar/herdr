...
### Fixed
- Codex panes now keep agent identity when downstream packages rename the executable: the dependency-free session-start hook binds identity to the live foreground job, while working, blocked, and idle state still comes from screen detection.
