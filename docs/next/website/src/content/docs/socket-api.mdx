...
Herdr-managed integrations may also send `reporter_pid`. For supported official source/agent pairs, the server can use that short-lived PID to bind identity to the reporter's live foreground job. The field does not grant state authority and is ignored for unsupported or custom sources.
