...
Codex フックは Herdr バイナリを直接使用するため、Python は必要ありません。セッション開始時に再開可能なセッションを報告し、Codex の識別情報をそのライブなフォアグラウンドジョブに結び付けるため、下流パッケージが実行ファイル名を変更してもエージェント検出は壊れません。`working`、`blocked`、`idle` の状態は引き続き Herdr のスクリーンマニフェスト検出から得られ、フックはライフサイクル状態を報告しません。
