...
Herdr が管理するインテグレーションは `reporter_pid` も送信できます。対応する公式のソース/エージェントの組み合わせでは、サーバーがこの短命な PID を使って、識別情報を報告元のライブなフォアグラウンドジョブに結び付けられます。このフィールドは状態の権限を与えず、未対応またはカスタムのソースでは無視されます。
