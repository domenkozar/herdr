...
The Codex hook uses the Herdr binary directly and does not require Python. On session start, it reports the resumable session and binds Codex identity to that exact live foreground job, so downstream packages can rename the executable without breaking agent detection. Working, blocked, and idle state still comes from Herdr's screen manifest detection; the hook does not report lifecycle state.
