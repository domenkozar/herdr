...
#### Per-tab rows inside a Space

`state_icon` and `state_text` on a Space row are a rollup: they collapse every
pane in the Space to the single most demanding state. To see each tab separately
instead, add `tab_rows`. It is empty by default, and each configured row renders
once per tab, nested under the Space:
...
Tab rows accept the Space tokens plus the agent-facing ones:

- `state_icon`, `state_text` — this tab's own agent state, rolled up across only
  that tab's panes.
- `tab` — tab name.
- `workspace` — workspace name.
- `agent` — agent display name from the tab's focused pane.
- `terminal_title`, `terminal_title_stripped` — terminal title from the tab's focused pane.
- `branch` — Git branch for the tab's own working directory.
- `git_status` — ahead and behind counts for the tab's own working directory.
- `$name` — custom pane metadata named `name`, from the tab's focused pane.

`branch` and `git_status` here resolve against each tab's own directory, which
matters because a tab created with `herdr tab create --cwd <path>` can live in a
different worktree, or a different repository, from the rest of its Space. Herdr
resolves per-tab Git values only while a `tab_rows` layout actually contains
`branch` or `git_status`; results for tabs sharing one repository are still
fetched once and shared. Until the first per-tab result arrives, a tab shows its
workspace's branch.

`pane` is not available in tab rows, since a tab may hold several panes.
...
### Hiding the Agent panel

`ui.sidebar.agents.show_header` hides the Agent panel's title and sort toggle.
To remove the panel altogether, set `ui.sidebar.agents.show`:
...
The Space list then takes the full sidebar height and the section divider
disappears, so there is nothing left to drag or click. This pairs naturally with
`tab_rows`, which already puts per-tab agent state in the Space list:

```toml
[ui.sidebar.spaces]
rows = [["state_icon", "workspace", "state_text"]]
tab_rows = [["state_icon", "branch"]]

[ui.sidebar.agents]
show = false
```

Unlike the row layouts, this one applies to the narrow sidebar too, which
otherwise shows its own compact agent list.
