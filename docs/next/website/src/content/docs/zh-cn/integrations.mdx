...
Codex 钩子直接使用 Herdr 二进制文件,不需要 Python。会话启动时,它会上报可恢复会话,并将 Codex 身份绑定到该实时前台作业,因此下游软件包即使重命名可执行文件也不会破坏智能体检测。`working`、`blocked` 和 `idle` 状态仍来自 Herdr 的屏幕清单检测;钩子不负责上报生命周期状态。
